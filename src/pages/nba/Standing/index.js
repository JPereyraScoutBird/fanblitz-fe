...
function Standing() {
  ...
  const [rankingTeamLeague, setRankingTeamLeague] = useState([{value: "Division", label: "Division"}, {value: "League", label: "League"}, {value: "NBA", label: "NBA"}]);
  ...
  const fetchData = async () => {
      try {
        const response = await axios.get('https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/rankingdata',
        );
  ...
  const fetchDataPlayer = async (tableOptions=null) => {
    try {
      ...
      const response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/players/ranking?season=${season}&position=${position}&league=${league}&division=${division}&team_id=${team}&option=${option}`,
      );
      console.log("URL PLAYER", `https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/players/ranking?season=${season}&position=${position}&league=${league}&division=${division}&team_id=${team}&option=${option}`)
      if (option == "Stats") {
        ...
        console.log("player data position", jsonObject)
      }
      setIsLoading(true);
    } catch (error) {
      console.error('Error getting data:', error);
  ...
  const fetchDataTeam = async (tableOptions=null) => {
    try {
      ...
      let division = divisionValue == null? "": divisionValue.value
      let option = tableOptions == null? showTableOptions: tableOptions
      const response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/teams/ranking?season=${season}&filter=${teamLeague}&league=${league}&option=${option}`,
      );
      // console.log("URL TEAM", `https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/teams/ranking?season=${season}&filter=${teamLeague}&league=${league}&division=${division}&option=${option}`)
      let jsonObject = response.data.body
  ...
  const onClickPlayer = (user) => {
    navigate(`/nba${PATH_LIST.PLAYER_DETAIL}/${user.player_id}`);
  }

  const onClickTeam = (user) => {
    navigate(`/nba${PATH_LIST.TEAM_DETAIL}/${user.team_id}`);
  }
    ...
    if (showTable === "Team") {
      ...
      if (showTableOptions === "Stats" && Object.keys(teamDataStats).length > 0){
        ...
        return sortedKeysStats.map((name, index) => (
          ...
          <h7>Points</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_pts'].length == 0} pagination={true} range={5} header={constant.headerTeamPTS} data={teamDataStats[name]['top_10_pts']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
          <h7>FIELD GOAL %</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_fg'].length == 0} pagination={true} range={5} header={constant.headerTeamFG} data={teamDataStats[name]['top_10_fg']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
          <h7>POINTS DIFFERENTIAL</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_plus_minus'].length == 0} pagination={true} range={5} header={constant.headerTeamDiff} data={teamDataStats[name]['top_10_plus_minus']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
          <br></br>
          <h5>Defensive Leaders</h5>
          <h7>POINTS ALLOWED</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_off_pts'].length == 0} pagination={true} range={5} header={constant.headerTeamPTSA} data={teamDataStats[name]['top_10_off_pts']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
          <h7>REBOUNDS</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_reb'].length == 0} pagination={true} range={5} header={constant.headerTeamReb} data={teamDataStats[name]['top_10_reb']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
          <h7>BLOCKS</h7>
          <CustomTable playeImage={true} search={false} search_placeholder="Search player name or team on table" search_keys={['full_name']} loading={teamDataStats[name]['top_10_blk'].length == 0} pagination={true} range={5} header={constant.headerTeamBlk} data={teamDataStats[name]['top_10_blk']} onClick={(user) => onClickTeam(user)}/>
          <br></br>
