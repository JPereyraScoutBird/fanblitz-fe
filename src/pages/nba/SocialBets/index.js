...
function SocialBets(props) {
  ...
  const [gameCBB, setGameDataCBB] = useState([]);
  const [gameNBA, setGameDataNBA] = useState([]);
  ...
  const fetchDataUser = async () => {
    try {
        const response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/user?user_id=${user.username}&username=${newFriendEmail}&action=search`);
        let jsonObject = response.data.body
        // console.log("users found URL", `https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/user?user_id=${user.username}&username=${newFriendEmail}&action=search`)
  ...
  const sendInvite = async (userFriend) => {
    try {
        if (userFriend!=undefined && userFriend.status_friend != "Following"){
          let response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/friends?user_id=${user.username}&friend_id=${userFriend.user_id}&action=create`);
          let jsonObject = response.data.body
          // console.log("sendInvite URL", `https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/friends?user_id=${user.username}&friend_id=${userFriend.user_id}&action=create`)
          // console.log("sendInvite response", jsonObject)
        }
        else{
          let response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/friends?user_id=${user.username}&friend_id=${userFriend.user_id}&action=delete`);
          let jsonObject = response.data.body
          console.log("DELETE URL", `https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/users/friends?user_id=${user.username}&friend_id=${userFriend.user_id}&action=delete`)
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get('https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/cbb/games', );
      const jsonObject = JSON.parse(response.data.body)
      const response_formated = jsonObject.length ? jsonObject.map(x => ({
        ...x,
        "date_z": getDate(x['date_z']),
      ...
      // console.log(response_formated)
      setGameDataCBB(response_formated)
    } catch (error) {
      console.error('Error getting data:', error);
    } 

    try {
      const response = await axios.get('https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/games', );
      const jsonObject = JSON.parse(response.data.body)
      const response_formated = jsonObject.length ? jsonObject.map(x => ({
        ...x,
        "date_z": getDate(x['date_z']),
      ...
      // console.log(response_formated)
      setGameDataNBA(response_formated)
    } catch (error) {
      console.error('Error getting data:', error);
    } 
  };
  ...
  const showGames = () => {
    ...
    if(sport == 1){
      ...
      return gameCBB.filter(x => getTodayItems(x.date_z)).map(y => {
        return (
          <option value={y.id}>
            {y.home_team_abbr} vs {y.away_team_abbr}
          </option>
      ...
      return gameNBA.filter(x => getTodayItems(x.date_z)).map(y => {
        return (
          <option value={y.id}>
            {y.home_team_abbr} vs {y.away_team_abbr}
          </option>
    ...
    <div id="socialbets">
      ...
      <Container>
        <div style={{ backgroundColor: "#fff", marginTop: "2rem" }}>
          ...
          <CustomTable noRange={false} pagination={true} range={10} header={constant.headerBets} data={bets ? 
          bets.map(x => ({...x, 
        ...
        <div style={{ backgroundColor: "#fff", marginTop: "2rem" }}>
          ...
          <CustomTable noRange={false} pagination={true} range={10} header={constant.headerBets2} data={bets ? betsFriend.map(x => ({...x, "sport": <><FontAwesomeIcon icon={renderImg(x.sport)} /> <span>{x.sport.toUpperCase()}</span></>, "fanduel":  <a onClick={toggle2}>See Bet</a>})): []} loading={betsFriend == undefined}/>
        </div>
        {/* <ChatComponent /> */}
      </Container>
