...
function Forecasts() {
  ...
  useEffect(() => {
    setIsLoading(false);
    axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/forecasts`).then((res) => {
            ...
            forecastData.length ? forecastData.filter(x => filterByDate(getDate(x.date_z), date.toDate())).map(forecast => (
              <Col xs={12} md={6}>
                <Card 
                  ...
                  linkTitle={`/nba${PATH_LIST.FORECAST_DETAIL}/${forecast.home_team_abb}-${forecast.away_team_abb}/${getDate2(forecast.date_z)}`}
