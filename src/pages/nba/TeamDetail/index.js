...
const getNewsSpecificPlayer = (player_name) => (
  `https://newsapi.org/v2/everything?q=(baseball) AND (${player_name}) +NOT+nba+NOT+racial+NOT+"how to watch"+NOT+aaa+NOT+delayed+NOT+sex+NOT+shooting+NOT+dies+NOT+minors+NOT+retires+NOT+retirement&searchin=title&excludedomains=espn.com,biztoc.com,denverpost.com,brobible.com,seattlepi.com,hypebeast.com,nypost.com,thedailybeast.com,dailymail.co.uk,rivals.com,trendhunter.com&language=en&pageSize=100&sortBy=publishedAt`
)
      ...
      return (
        <div className='w-100 d-flex flex-column align-items-center'>
          <CardTeamComponent 
            ...
            link={`/nba${PATH_LIST.FORECAST_DETAIL}/${data.games[0]['home_team']}-${data.games[0]['away_team']}/${getDate2(data.games[0].date_z)}`}
          />
        </div>
      )
    }
...
function TeamDetail(props) {
    ...
    const [rankingSeason, setRankingSeason] = useState([{value: "2023-2024-regular", label: "2023-2024-regular"}]);
    const [currentSeason, setCurrentSeason] = useState({value: "2023-2024-regular", label: "2023-2024-regular"}); 

    useEffect(() => {
        const fetchData2 = async () => {
            try {
                const response = await axios.get(`https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/teams?id=${teamId}`);
    ...
    const renderTableLeaders = (data, seasonValue, backgroundColor = "#000", color='#fff') => {
      ...
      return (
        ...
        {data_filter.map(x => (
          <CardProfileComponent link={`/nba${PATH_LIST.PLAYER_DETAIL}/${x.player_id}`} title={`${x['first_name'][0]}. ${x['last_name']}`} className="col-2" body={<div className="w-100 d-flex justify-content-around"><span>{x['position']}.</span> <span>{x.feature}: <strong>{x.value}</strong></span></div>} imageSrc={x['image'] ||  Image.PROFILE}/>
        ))}
      </Row>
      )
    };
      ...
      return (
        <div style={{}}>
          ...
          <Modal isOpen={modal} toggle={toggle2}>
            <Chatbot player={teamDetail.odds_api} pre_prompt={`${teamDetail.odds_api}'nba basketball team history`} gptStyle={gptStyle}/>
          </Modal>
        </div>    
      );
    }
    ...
    return (
      ...
      {/* <div id="template"> */}
        ...
        { teamDetail ? <SubMenu home={`/nba${PATH_LIST.TEAM_DETAIL}/:${teamDetail.id}}`} links={constant.links} wins={teamDetail.wins} losses={teamDetail.losses} backgroundColor={"#041e42" || "#000"} color={"#fff"} logo={''} /> : null}
