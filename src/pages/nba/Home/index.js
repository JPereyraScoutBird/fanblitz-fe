...
function Home(props) {
  ...
  const fetchData = async () => {
      try {
        ...
        // console.log("Data GAMES: ", jsonObject)
  ...
  const fetchDataScoreData = async () => {
    try {
      const responseModel = await axios.get('https://crfh3pd7oi.execute-api.us-east-1.amazonaws.com/qanba/nba/stats/teams/score');
  ...
  const renderForecastComponent = (game) => {
    if(true){
      return <CardForecastComponent
              ...
              title={`${game.home_team_city} vs. ${game.away_team_city}`}
              ...
              footer={<Link to={`/nba${PATH_LIST.FORECAST_DETAIL}/${game.home_team_abbr}-${game.away_team_abbr}/${getDate2(game.date_z)}`} className="btn btn-outline-light" outline={true}>FanBlitz Analysis</Link>}
            />
    }
    return <></>
  }
  ...
  const onClick2 = (game) => {
    ...
    setTeam(new_team2)
    setPrompt(`${new_team2}' NBA basketball team history`)
    toggle()
  }
  ...
  const onClick5 = (game) => {
    navigate(`/nba${PATH_LIST.FORECAST_DETAIL}/${game.home_team_abbr}-${game.away_team_abbr}/${getDate2(game.date_z)}`);
  }
  ...
  const onClickLogin = () => {
    navigate(`/nba${PATH_LIST.SOCIAL_BETS}`);
  }

  const renderTable = (table, data, date, backgroundColor = "#000", color='#fff') => {
      if(table == 'score') {
        let dataAux = data["scoremodel"].length ? data["scoremodel"].map(x => ({...x, "bet_icon": <a onClick={setBets}>Set a Bet</a>})) : []
  ...
  return (
    <div id="home">
      <Menu sport_default={"nba"} signOut={signOut} user={user} onChange={(e) => setGptStye(e)}/>
        {/* {renderCards()} */}
        {
      ...
      <Container>
        {
          isLoading == false ? getLoading() : renderTable(showTable, {"home": gameData, "scoremodel": pitcherStrikeoutData}, date)
        }
      </Container>
      <Footer />  
        ...
        <ModalBody>
            ...
            <FormGroup className="mb-2 mr-sm-2 mb-sm-0">
              ...
              <Input
                ...
                type="select"
              >
                <option value={3} hidden>NBA</option>
                <option value={3}>
                  NBA
                </option>
              </Input>
