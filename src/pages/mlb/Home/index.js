...
function Home(props) {
  ...
  const [date, setDate] = useState(moment(new Date('2023-11-02').toLocaleString('en-US', {timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone}) ))
  ...
  const renderCards = () => {
    if (gameData != undefined && gameData.length > 0) {
      const today_games = gameData.filter(x => getTodayItems(x.date_z, '2023-11-02'))
